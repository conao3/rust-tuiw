...
async fn test_tmux_session_lifecycle() {
    ...
    wrapper::create_session(session_name, "bash", "/tmp")
        .await
    ...
    wrapper::send_keys(session_name, "echo hello")
        .await
        .expect("failed to send keys");

    wrapper::send_keys(session_name, "Enter")
        .await
    ...
    wrapper::kill_session(session_name)
        .await
